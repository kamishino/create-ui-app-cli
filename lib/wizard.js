  ...
  while (addMore) {
    const response = await prompts([
      ...
      },
      {
        ...
        validate: (value) => {
          // Allow "user/repo" format or full URLs
          if (value.includes("http") || value.includes("git@") || value.split("/").length === 2) {
            return true;
          }
          return "Please use 'user/repo' format or a valid Git URL";
        },
      },
      {
        type: "text",
...
async function manageTemplates(config) {
  ...
  switch (action) {
    case "add":
      ...
      break;
    case "view":
      const templatesToView = config.get("templates");
      if (templatesToView && templatesToView.length > 0) {
        console.log(chalk.cyan("\nCurrent templates:\n"));
        templatesToView.forEach((t, i) => {
      ...
      break;
    case "remove":
      const currentTemplates = config.get("templates");
      if (!currentTemplates || currentTemplates.length === 0) {
        console.log(chalk.yellow("\nNo templates to remove.\n"));
        break;
      }

      const { templateToRemove } = await prompts({
        type: "select",
        name: "templateToRemove",
        message: "Select a template to remove:",
        choices: currentTemplates.map((t, i) => ({
          title: t.title,
          value: i,
          description: t.value.repo,
        })),
      });

      if (templateToRemove !== undefined) {
        const removed = currentTemplates.splice(templateToRemove, 1);
        config.set("templates", currentTemplates);
        console.log(chalk.green(`\n✓ Removed template: ${removed[0].title}\n`));
      } else {
        console.log(chalk.yellow("\nCancelled.\n"));
      }
      break;
